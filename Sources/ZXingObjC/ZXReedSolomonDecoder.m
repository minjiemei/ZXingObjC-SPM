    ...
    for (int i = 0; i < errorLocations.length; i++) {
        ...
        if (position < 0) {
            NSDictionary *userInfo = @{
                    NSLocalizedDescriptionKey: @"Bad error location"
            };

            if (error) {
                *error = [[NSError alloc] initWithDomain:ZXErrorDomain code:ZXReedSolomonError userInfo:userInfo];
            }
...
- (NSArray *)runEuclideanAlgorithm:(ZXGenericGFPoly *)a b:(ZXGenericGFPoly *)b R:(int)R error:(NSError **)error {
    ...
    while ([r degree] >= R / 2) {
        ...
        if ([rLast zero]) {
            NSDictionary *userInfo = @{
                    NSLocalizedDescriptionKey: @"r_{i-1} was zero"
            };
    ...
    if (sigmaTildeAtZero == 0) {
        NSDictionary *userInfo = @{
                NSLocalizedDescriptionKey: @"sigmaTilde(0) was zero"
        };
...
- (ZXIntArray *)findErrorLocations:(ZXGenericGFPoly *)errorLocator error:(NSError **)error {
    ...
    if (e != numErrors) {
        NSDictionary *userInfo = @{
                NSLocalizedDescriptionKey: @"Error locator degree does not match number of roots"
        };
